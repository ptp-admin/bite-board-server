...
recipesRouter.get('/', (req, res) => {
  ...
  db
    ...
    .from('recipe')
    .then(recipes => {
      const whereInClause = recipes.map(recipe => {
        return "'" + recipe.id + "'";
      }).join(',');
      ...
      db
        .raw(`
					SELECT
						...
						ri.number_of as recipe_ingredient_number_of,
						ri.measurement_unit as recipe_ingredient_measurement_unit,
						ri.recipe_id
					FROM
        ...
        .then(result => {
          ...
          return recipes.map(recipe => {
            return {
              ...recipe,
              ingredients: ingredients.filter(ingredient => ingredient.recipe_id === recipe.id).map(ingredient => {
                // destructure the properties returned by the above SQL query
                const { ingredient_cost_per, ingredient_number_of, recipe_ingredient_number_of, recipe_ingredientMeasurementUnit, ...rest } = ingredient;
                // reassemble the desired ingredient object
                return {
                  ...rest,
                  recipe_ingredient_number_of,
                  measurement_unit: recipe_ingredientMeasurementUnit,
                  ingredient_derived_cost: deriveCost(ingredient_cost_per, ingredient_number_of, recipe_ingredient_number_of) // TODO: write a function which handles this, calculateDerivedCost(ingredient). Account for cases where ingredient.measurement_unit and recipe_ingredient.measurement_unit need to be converted to calculate cost.
                };
              })
            };
          });
...
// call it deriveCost()  ??
const deriveCost = (ingredient_cost_per, ingredient_number_of, recipe_ingredient_number_of) => {
  return (ingredient_cost_per / ingredient_number_of) * recipe_ingredient_number_of;
};
